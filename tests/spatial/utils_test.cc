...
TEST_CASE("Spatial utills check", "[spatial][utils]") {
  ...
  std::vector<std::shared_ptr<Point>> dst_points{
      std::make_shared<Point>(Point(0, "spatial0", {3.1, 3.9}, "residential0")),
  ...
  SECTION("Map two points list") {
    auto map = cityscape::spatial::map_objects(src_points, dst_points);
  ...
  SECTION("Map points to segment") {
    auto map = cityscape::spatial::map_objects(dst_points, segments);
  ...
  SECTION("Merge two vectors of points") {
    double thre = 0.15;
    auto leftovers =
        cityscape::spatial::merge_objects(src_points, dst_points, thre);
